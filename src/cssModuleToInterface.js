...
const cssModuleToTypescriptInterfaceProperties = (
  cssModuleKeys,
  indent = "  "
) => {
...
export const filterNonWordClasses = cssModuleKeys => {
  const filteredClassNames = cssModuleKeys.filter(classname =>
    allWordsRegexp.test(classname)
  );
  if (filteredClassNames.length === cssModuleKeys.length) {
    return [filteredClassNames, []];
  }
  const nonWordClassNames = cssModuleKeys.filter(
    classname => !allWordsRegexp.test(classname)
  );
...
export const filterReservedWordClasses = cssModuleKeys => {
  const filteredClassNames = cssModuleKeys.filter(
    classname => reservedWords.indexOf(classname) === -1
  );
  if (filteredClassNames.length === cssModuleKeys.length) {
    return [filteredClassNames, []];
  }
  const reservedWordClassNames = cssModuleKeys.filter(
    classname => reservedWords.indexOf(classname) !== -1
  );
...
export const generateGenericExportInterface = (
  cssModuleKeys,
  filename,
  indent
) => {
